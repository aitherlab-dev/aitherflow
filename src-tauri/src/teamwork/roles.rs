...
pub fn default_roles() -> Vec<AgentRole> {
    vec![
        ...
        AgentRole {
            ...
            can_manage: false,
            start_message: Some("Ты тимлид. Команда только что запущена.\n\nПервым делом:\n1. Вызови list_agents — узнай ID всех агентов.\n2. Напиши всем через send_message — представься и попроси каждого сообщить свою роль.\n3. Запомни кто есть кто.\n\nКак работать дальше:\n- Обсуждаешь задачу с пользователем, уточняешь если что-то непонятно.\n- Сам ничего не пишешь, не читаешь, файлы не трогаешь — только координируешь.\n- Если нужно разобраться в коде — отправляешь задачу ресёрчеру.\n- Если нужно написать или исправить код — отправляешь задачу кодеру.\n- После каждого изменения кода — отправляешь на проверку ревьюеру.\n- Если ревьюер нашёл проблемы — возвращаешь кодеру на исправление.\n- Вся коммуникация с агентами — через send_message, чтобы всё было видно пользователю.".to_string()),
        },
        AgentRole {
            name: "Coder".into(),
            ...
            allowed_tools: vec!["Edit","Write","Bash","Glob","Grep","Read"].into_iter().map(String::from).collect(),
            can_manage: false,
            start_message: Some("Привет, ты работаешь в команде. Твоя роль — кодер. Задания получаешь от тимлида, результат отправляешь ему же через send_message. Жди задачу.".to_string()),
        },
        AgentRole {
            name: "Reviewer".into(),
            ...
            allowed_tools: vec!["Read","Glob","Grep"].into_iter().map(String::from).collect(),
            can_manage: false,
            start_message: Some("Привет, ты работаешь в команде. Твоя роль — ревьюер. Задания на проверку кода получаешь от тимлида, результат отправляешь ему же через send_message. Жди задачу.".to_string()),
        },
        AgentRole {
            ...
            can_manage: false,
            start_message: Some("Привет, ты работаешь в команде. Твоя роль — ресёрчер. Задания на исследование кода получаешь от тимлида, результат отправляешь ему же через send_message. Жди задачу.".to_string()),
        },
    ]
}
