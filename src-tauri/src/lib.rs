...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            conductor::get_subscription_usage,
            config::get_workspace_path,
            chats::list_chats,
            chats::list_chats_by_activity,
            chats::list_recent_chats_all,
            chats::create_chat,
            chats::load_chat,
            ...
            telegram::commands::telegram_send_resume_projects,
            telegram::commands::telegram_send_team_projects,
            telegram::commands::telegram_send_skills,
            telegram::commands::telegram_send_chats,
            telegram::commands::telegram_send_recent_chats,
                ...
                .on_menu_event(move |app_handle, event| match event.id().as_ref() {
                    "show" => {
                        if let Some(window) = app_handle.get_webview_window("main") {
                            window.show().map_err(|e| eprintln!("[tray] Failed to show: {e}")).ok();
                            window.set_focus().map_err(|e| eprintln!("[tray] Failed to focus: {e}")).ok();
                        }
                    }
                    "quit" => {
                    ...
                    } = event
                    {
                        ...
                        if let Some(window) = app_handle.get_webview_window("main") {
                            if window.is_visible().unwrap_or(false) {
                                window.hide().map_err(|e| eprintln!("[tray] Failed to hide: {e}")).ok();
                            } else {
                                window.show().map_err(|e| eprintln!("[tray] Failed to show: {e}")).ok();
                                window.set_focus().map_err(|e| eprintln!("[tray] Failed to focus: {e}")).ok();
                            }
