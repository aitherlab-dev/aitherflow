...
import type { Provider } from "../../types/external-models";
    ...
    <div className="settings-section-general">
      ...
      {PROVIDERS.map(({ id, label }) => (
        <ProviderBlock
          ...
          state={providers[id]}
          showKey={showKeys[id]}
          onToggleShowKey={() => actions.toggleShowKey(id)}
          onUpdate={(patch) => actions.updateProvider(id, patch)}
          onTest={() => actions.testConnection(id)}
          onLoadModels={() => actions.loadModels(id)}
          mgmtKey={id === "openrouter" ? mgmtKey : undefined}
          showMgmtKey={id === "openrouter" ? showMgmtKey : undefined}
          onToggleShowMgmtKey={id === "openrouter" ? () => actions.setShowMgmtKey((v: boolean) => !v) : undefined}
          onMgmtKeyChange={id === "openrouter" ? actions.handleMgmtKeyChange : undefined}
        />
      ))}
